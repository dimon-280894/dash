...
*
*   Why is this needed?
*   - In Simplecointest some blocks are superblocks, which output much higher amounts of coins
...
{
    if(fLiteMode) return; // disable all Simplecointest specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f SIMPLECOINTEST\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
