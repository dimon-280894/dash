...
const char * const BITCOIN_CONF_FILENAME = "simplecointest.conf";
const char * const BITCOIN_PID_FILENAME = "simplecointestd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "simplecointest" is a composite category enabling all Simplecointest-related debug output
            if(ptrCategory->count(string("simplecointest"))) {
...
#else
    const char* pszModule = "simplecointest";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SimplecointestCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SimplecointestCore
    // Mac: ~/Library/Application Support/SimplecointestCore
    // Unix: ~/.simplecointestcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SimplecointestCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SimplecointestCore";
#else
    // Unix
    return pathRet / ".simplecointestcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty simplecointest.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override simplecointest.conf
